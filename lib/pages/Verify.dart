...
@override
  ...
  sendverifylink()async{
    ...
    await user.sendEmailVerification().then((value)=>{  // abh phale user ki mail pr verification link sent krenge to verify then ek popup-type show hoga link send ke leye
      Get.snackbar('Link sent','Link has been sent to you mail')
    });
  }

  reload()async{
    await FirebaseAuth.instance.currentUser!.reload().then((value)=>{ Get.offAll(Wrapper() as Widget Function())});  //after email verification and user click on button then all the screen will get remove and user ko wrapper page pe load kra denge and now again wrapper decide krega agar data hai then mail verify hai?-> home page else login page
  }
